...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen)
{
    ...
    struct tcp *tcp = TCP_HDR_FROM_DATA(buffer);
    tcp->ack = 0;
    tcp->seq = sock->initial_seq;
    tcp->checksum = do_tcp_csum(
        (void *)tcp,
        TCP_HLEN,
        IPPROTO_TCP, 
        htonl(CLIENT_IP), 
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags)
{
    ...
    // the default path, should never be the case?
